...
def get_company():
    ...
    if symbol is None:
        return Response(json.dumps({"error": "missing 'ticker' query-parameter"}), status=400,
...
def get_product_and_price_average():
    ...
    if query is None:
        return Response(json.dumps({"error": "missing 'search_text' query-parameter"}), status=400,
                        mimetype='application/json')
